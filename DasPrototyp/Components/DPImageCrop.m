...
static const NSTimeInterval kDefualRatioOfWidthAndHeight = 1.0f;

@interface UIImage (DPImageCrop_Addition)
...
@implementation UIImage (DPImageCrop_Addition)
...
@interface DPImageCrop ()<UIScrollViewDelegate>
...
- (void)viewDidLoad {
  ...
  //绘制俩button
  [self.view addSubview:self.cancelButton = [self acquireCustomButtonWithTitle:NSLocalizedString(@"Cancel", @"")
                                                                     andAction:@selector(onCancel:)]];
  [self.view addSubview:self.confirmButton = [self acquireCustomButtonWithTitle:NSLocalizedString(@"OK", @"")
                                                                      andAction:@selector(onConfirm:)]];
  //双击事件
...
- (UIButton *)acquireCustomButtonWithTitle:(NSString*)title
...
- (void)onConfirm:(id)sender {
  ...
  if (self.scrollView.tracking
      ...
      || self.scrollView.zoomBouncing
      || self.scrollView.zooming) {
    return;
  }
  //根据区域来截图
  CGPoint startPoint = [self.overlayView convertPoint:CGPointZero
  ...
  CGRect cropRect = CGRectMake(startPoint.x*wRatio, startPoint.y*hRatio,
                               (endPoint.x-startPoint.x)*wRatio,
                               (endPoint.y-startPoint.y)*hRatio);
  [self disappear];
  ...
  if (self.imageView.image) {
    ...
    if (isBaseOnWidth) {
      self.scrollView.contentInset = UIEdgeInsetsMake(CGRectGetMinY(self.overlayView.frame),
                                                      0, CGRectGetHeight(self.view.bounds)-CGRectGetMaxY(self.overlayView.frame), 0);
    }else{

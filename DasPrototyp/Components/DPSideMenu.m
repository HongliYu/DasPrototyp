...
static const NSTimeInterval kAnimationDelay = 0.08f;

typedef CGFloat (^EasingFunction)(CGFloat, CGFloat, CGFloat, CGFloat);

@interface DPSideMenu()
...
- (id)initWithItems:(NSArray *)items {
  ...
  if (self) {
    ...
    _menuPosition = DPSideMenuPositionRight;
  }
  return self;
}
...
- (void)close {
  ...
  for (UIView *item in self.items) {
    [self performSelector:@selector(hideItem:)
               withObject:item
               afterDelay:kAnimationDelay * [self.items indexOfObject:item]];
  }
}
...
- (void)showItem:(UIView *)item {
  ...
  if (self.menuIsVertical) {
    position.x += self.menuPosition == DPSideMenuPositionRight ? - self.menuWidth : self.menuWidth;
    [self animateLayer:item.layer
           withKeyPath:@"position.x"
                    to:position.x];
  } else {
    position.y += self.menuPosition == DPSideMenuPositionTop ? self.menuHeight : - self.menuHeight;
    [self animateLayer:item.layer
...
- (void)hideItem:(UIView *)item {
  ...
  item.layer.position = position;
  [item.layer performSelector:@selector(setOpacity:)
                   withObject:[NSNumber numberWithFloat:0.0f]
                   afterDelay:0.5];
}

- (BOOL)menuIsVertical {
  if (self.menuPosition == DPSideMenuPositionLeft
      || self.menuPosition == DPSideMenuPositionRight) {
    return YES;
  }
  return NO;
}
...
- (void)layoutSubviews {
  ...
  if (self.menuIsVertical) {
    x = self.menuPosition == DPSideMenuPositionRight ? self.superview.frame.size.width : 0 - self.menuWidth;
    ...
    y = self.menuPosition == DPSideMenuPositionTop ? 0 - self.menuHeight : self.superview.frame.size.height;
  }
  ...
  [self.items enumerateObjectsUsingBlock:^(UIView *item, NSUInteger idx, BOOL *stop) {
    if (self.menuIsVertical)
      [item setCenter:CGPointMake(itemInitialX, (idx * biggestHeight)
                                  + (idx * self.itemSpacing) + (biggestHeight / 2))];
    else
      [item setCenter:CGPointMake((idx * biggestWidth)
                                  + (idx * self.itemSpacing) + (biggestWidth / 2), self.menuHeight / 2)];
  }];
}
...
- (void)animateLayer:(CALayer *)layer
  ...
  for (CGFloat t = 0; t < steps; t++) {
    [values addObject:@(function(animation.duration * (t / steps),
